...
from .ashare_semantics import (
    build_joinquant_ashare_policy,
    is_joinquant_ashare_limit_threshold,
)
...
class Exchange:
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        ashare_limit_options = kwargs.pop("ashare_limit_options", None)
        ashare_price_limit_mode = kwargs.pop("ashare_price_limit_mode", None)
        self._joinquant_ashare_policy = None
        if ashare_price_limit_mode is not None:
            ashare_limit_options = {
                **(ashare_limit_options or {}),
                "price_limit_mode": ashare_price_limit_mode,
            }
        ...
        if is_joinquant_ashare_limit_threshold(limit_threshold):
            self._joinquant_ashare_policy = build_joinquant_ashare_policy(ashare_limit_options)
            self.trade_unit = self._joinquant_ashare_policy.trade_unit if self.trade_unit is None else self.trade_unit
        if deal_price is None:
            deal_price = self._joinquant_ashare_policy.deal_price if self._joinquant_ashare_policy else C.deal_price
        ...
        if self._joinquant_ashare_policy is not None:
            necessary_fields.add(self._joinquant_ashare_policy.up_limit_field)
            necessary_fields.add(self._joinquant_ashare_policy.down_limit_field)
    ...
    LT_JOINQUANT_ASHARE = "joinquant_ashare"  # string preset: authoritative A-share daily price-limit bounds
    ...
    def _get_limit_type(self, limit_threshold: Union[tuple, float, str, None]) -> str:
        ...
        if isinstance(limit_threshold, list):
            ...
            return self.LT_FLT
        elif is_joinquant_ashare_limit_threshold(limit_threshold):
            return self.LT_JOINQUANT_ASHARE
    ...
    def _update_limit(self, limit_threshold: Union[Tuple, float, str, None]) -> None:
        ...
        elif limit_type == self.LT_JOINQUANT_ASHARE:
            if self._joinquant_ashare_policy is None:
                self._joinquant_ashare_policy = build_joinquant_ashare_policy()
            self.quote_df = self._joinquant_ashare_policy.apply_price_limits(
                self.quote_df,
    ...
    def _calculate_trade_cost(
        ...
        if self._joinquant_ashare_policy is not None:
            return self._joinquant_ashare_policy.calculate_trade_cost(
                "sell" if direction == Order.SELL else "buy",
                trade_val,
        ...
        **NOTE**: Order will be changed in this function
        ...
        if order.direction == Order.SELL:
            ...
            if position is not None:
                ...
                current_amount = (
                    position.get_sellable_amount(order.stock_id) if position.check_stock(order.stock_id) else 0
                )
                ...
                expected_trade_val = order.deal_amount * trade_price
                expected_trade_cost = self._calculate_trade_cost(
                    order.direction,
                    expected_trade_val,
                    cost_ratio,
                    adj_cost_ratio,
                )
                if position.get_cash() + expected_trade_val < expected_trade_cost:
                    order.deal_amount = 0
        ...
        trade_val = order.deal_amount * trade_price
        trade_cost = self._calculate_trade_cost(order.direction, trade_val, cost_ratio, adj_cost_ratio)
        return trade_price, trade_val, trade_cost
